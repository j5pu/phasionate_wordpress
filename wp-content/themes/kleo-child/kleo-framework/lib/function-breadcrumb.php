...
function kleo_breadcrumb( $args = array() ) {
	...
	$defaults = array(
		...
		'after'      => false,
		'front_page' => true,
		'show_home'  => __( 'Phasionate', 'kleo_framework' ),
		'network'    => false,
		'echo'       => false
	);
...
function breadcrumb_trail_get_buddypress_items($args = array()) 
{
	...
	if ( !empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
		$trail[] ='<a href="' . bp_get_members_directory_permalink() . '">' . get_the_title( $bp->pages->members->id ) . '</a>';
  ...
  $trail[] = '<a href="'. get_permalink( $bp->pages->{$bp->current_component}->id ) .'" title="'. esc_attr( strip_tags( get_the_title( $bp->pages->{$bp->current_component}->id ) ) ) .'">'. get_the_title( $bp->pages->{$bp->current_component}->id ) .'</a>';
		...
		$trail[] = '<a href="' . get_permalink( $bp->pages->groups->id ) . '" title="' . esc_attr( get_the_title( $bp->pages->groups->id ) ) . '">' . get_the_title( $bp->pages->groups->id ) . '</a>';

		$trail_end = __( 'Create a Group', 'buddypress' );
		
	} else if ( bp_is_create_blog() ) {
		$trail[] = '<a href="'.  home_url().'/'. $bp->current_component .'" title="'. ucwords($bp->current_component) .'">'. ucwords($bp->current_component) .'</a>';
