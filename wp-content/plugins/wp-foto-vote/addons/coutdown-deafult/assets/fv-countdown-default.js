		...
		var image = jQuery('<img />').load(function() {
			...
			function cf_draw(from, to, canvas, step) {
				var context = canvas.getContext('2d'),
					...
					dig_col = parseInt(jQuery(canvas).parents('div.c-block')[0].className.substr(-1), 10);
			...
			function seconds(from, canvas) {
				...
				if (from <= 0) {
					...
					// minutes animate
					var minutes_from = parseInt(jQuery('span', minutes_cnt).text(), 10);
					...
					if (minutes_from <= 0) {
						...
						// hourses animate
						var hourses_from = parseInt(jQuery('span', hourses_cnt).text(), 10);
						var hourses_to = hourses_from - 1;
						
						if (hourses_from <= 0) {
							hourses_to = 23;
							
							var days_from = parseInt(jQuery('span', days_cnt).text(), 10);
							var days_to = days_from - 1;
							
							if (days_from == 0) {
								return;
							}
							cf_animate(days_from, days_to, jQuery('canvas', days_cnt)[0], 0);
							jQuery('span', days_cnt).text(days_to);
						}
						cf_animate(hourses_from, hourses_to, jQuery('canvas', hourses_cnt)[0], 0);
						jQuery('span', hourses_cnt).text(hourses_to);
					}
	
					cf_animate(minutes_from, minutes_to, jQuery('canvas', minutes_cnt)[0], 0);
					jQuery('span', minutes_cnt).text(minutes_to);
				}
			...
			jQuery('div.bl-inner').each(function() {
				var canvas = jQuery('<canvas></canvas>').css({
					position: 'absolute',
					left: -5,
					top: 0
				}).appendTo(this)[0];
				
				canvas.width = jQuery(this).width() + offset_left * 2 + 3;
				canvas.height = jQuery(this).height();
