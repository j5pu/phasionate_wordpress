...
class FV_List_Votes_Log extends FV_WP_List_Table
{
    ...
    function extra_tablenav($which)
    {
        if ($which == "top") {
            ...
            if (isset($_GET['contest_id']) && $_GET['contest_id'] > 0) {
                ...
                $photos = ModelCompetitors::query()->where('contest_id', $selected_id)->find();
                ...
                <select name="contest-filter-photo"
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($photos as $P): ?>
                            <option
                                value="<?php echo $P->id ?>" <?php echo selected($P->id, $selected_photo_id) ?> ><?php echo $P->id, ' / ', $P->name ?></option>
                        <?php endforeach; ?>
                    <?php endif ?>
                </select>
            </div>
    ...
    function __construct()
    {
        ...
        parent::__construct(array(
            'singular' => 'votes_log', //singular name of the listed records
            'plural' => 'votes_logs', //plural name of the listed records
    ...
    function column_contest_id($item)
    {
        ...
        //Build row actions
        $actions = array(
            'delete' => sprintf('<a href="?page=%s&action=%s&votes_log=%s">Delete</a>', $_REQUEST['page'], 'delete', $item->id),
        );
    ...
    function prepare_items()
    {
         ...
         */
        $contest_id = (!empty($_REQUEST['contest_id'])) ? (int)$_REQUEST['contest_id'] : false; //If no sort, default to false
        ...
        $query = ModelVotes::query()
            ...
            ->leftJoin( ModelCompetitors::tableName(), "competitor", "`t`.`vote_id` = `competitor`.`id`", array("name") );
