        ...
        if ( 	(fv.security_type == "default" || fv.security_type == "cookieAregistered" || fv.security_type == "defaultAfb")
            ...
            $jQ.post(fv.ajax_url, send_data,
                function (data) {
					...
					msg = FvLib.applyFilters('fv/vote/modal_msg', msg);
...
function  fv_before_start_voting(el, action) {
    ...
    if ( fv.security_type == "defaultArecaptcha" || fv.security_type == "cookieArecaptcha" ) {
        ...
        $jQ.get(fv.ajax_url, { 'action': 'fv_is_subscribed', 'contest_id': fv.contest_id, 'post_id': fv.post_id, 'uid': fv_uid, 'fuckcache': FvLib.randomStr(8) },
            function (data) {
...
function sv_vote_send(service, el, id, for_vote) {
    ...
    switch (service) {
        case 'fb':
            ...
            if ( typeof(fv_fb) !== 'undefined' && fv_fb.appId.length > 3) {
                FB.ui({
                    method: 'feed',
					display: 'popup',
                    link: fv_url,
        ...
        function (response) {
            ...
            if ( response.result == "ok" ) {
                ...
                if ( !infinite ) {
                    ...
                    jQuery('html, body').animate({
                        scrollTop: $photos_container.offset().top - 50
                    }, 500);
                    ...
                    fv.page_url = response.share_page_url;
                    jQuery('html, body').animate({
                        scrollTop: $photos_container.offset().top + $photos_container.height() - 200
                    }, 500);
                }
...
/**
 * Function used for run social authorization
 * Need for did't show standard Ulogin.ru window, it looks not good
 */
function fv_soc_autorization(service, el) {
    switch (service) {
        case 'fb':
            $jQ('div[data-uloginbutton="facebook"]').click();
            break;
        case 'tw':
            $jQ('div[data-uloginbutton="twitter"]').click();
            break;
        case 'vk':
            $jQ('div[data-uloginbutton="vkontakte"]').click();
            break;
        case 'ok':
            $jQ('div[data-uloginbutton="odnoklassniki"]').click();
            break;
        case 'gp':
            $jQ('div[data-uloginbutton="google"]').click();
            break;
        default:
            return false;
    }
    ...
    jQuery.getJSON("//ulogin.ru/token.php?host=" +
        ...
        function (data) {
            ...
            if (!data.error) {
                ...
                //alert("Привет, "+data.first_name+" "+data.last_name+' '+data.email+"!");
                //console.log(data);
            }
        });
}
...
FvLib.addHook('fv/start_voting', start_voting_callback, 10, 3);

function start_voting_callback(fv_security_type, subscribed, action) {
	 console.log(fv_security_type);
}

// Filter example:

FvLib.addFilter('fv/vote/send_data', send_data_filter_callback);

function send_data_filter_callback(send_data) {
	send_data = 'test321';
	console.log( send_data );
	return send_data;
}
