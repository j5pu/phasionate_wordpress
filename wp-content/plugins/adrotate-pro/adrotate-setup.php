...
function adrotate_uninstall_setup() {
	...
	delete_option('adrotate_roles'); // Obsolete in 3.13.1
...
function adrotate_check_config() {
	...
	if(!isset($notifications['notification_dashboard']) OR ($notifications['notification_dashboard'] != 'Y' AND $notifications['notification_dashboard'] != 'N')) $config['notification_dashboard'] = 'Y';
...
function adrotate_database_upgrade() {
	...
	if($adrotate_db_version['current'] < 50) {
		...
		$budgets = $wpdb->get_results("SELECT `id`, `budget`, `ibudget` FROM {$wpdb->prefix}adrotate WHERE `ibudget` > 0 ORDER BY `id` ASC;");
		foreach($budgets as $budget) {
			$newbudget = $budget->budget + $budget->ibudget;
			$wpdb->update("{$wpdb->prefix}adrotate", array('budget' => $newbudget), array('id' => $budget->id));
			unset($budget);
		}
	...
	if($adrotate_db_version['current'] < 51) {
		adrotate_add_column("{$wpdb->prefix}adrotate", 'mobile', 'varchar(2) NOT NULL default \'Y\' AFTER `tracker`');
...
function adrotate_core_upgrade() {
	...
	if($adrotate_version['current'] < 350) {
		update_option('adrotate_debug', array('general' => false, 'publisher' => false, 'advertiser' => false));
	}
	...
	if($adrotate_version['current'] < 353) {
		if(!is_dir(ABSPATH.'/wp-content/plugins/adrotate-pro/language')) {
			mkdir(ABSPATH.'/wp-content/plugins/adrotate-pro/language', 0755);
		}
	}
	...
	if($adrotate_version['current'] < 380) {
		...
		update_option('adrotate_debug', array('general' => false, 'publisher' => false, 'advertiser' => false, 'geo' => false, 'timers' => false, 'track' => false));
	}
