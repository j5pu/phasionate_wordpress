...
<h3><?php _e('Your Active Ads', 'adrotate-pro'); ?></h3>
<p><em><?php _e('These are active and currently in the pool of ads shown on the website.', 'adrotate-pro'); ?></em></p>
...
?>
	    <tr id='banner-<?php echo $ad['id']; ?> <?php echo $ad['type']; ?>' class='<?php echo $class.$errorclass; ?>'>
			...
			<td>
				<strong><?php if($adrotate_config['enable_editing'] == 'Y') { ?><a class="row-title" href="<?php echo admin_url('/admin.php?page=adrotate-advertiser&view=edit&ad='.$ad['id']);?>" title="<?php _e('Edit', 'adrotate-pro'); ?>"><?php echo stripslashes(html_entity_decode($ad['title']));?></a><?php } else { echo stripslashes(html_entity_decode($ad['title'])); } ?></strong>
				<?php if($adrotate_config['stats'] == 1) { ?> - <a href="<?php echo admin_url('/admin.php?page=adrotate-advertiser&view=report&ad='.$ad['id']);?>" title="<?php _e('Stats', 'adrotate-pro'); ?>">Stats</a><?php } ?>
				<span style="color:#999;">
					<?php if($ad['crate'] > 0 OR $ad['irate'] > 0) {
						echo '<br /><span style="font-weight:bold;">'.__('Budget:', 'adrotate-pro').'</span> '.number_format($ad['budget'], 2, '.', '').' - '; 
						echo __('CPC:', 'adrotate-pro').' '.number_format($ad['crate'], 2, '.', '').' - ';
						echo __('CPM:', 'adrotate-pro').' '.number_format($ad['irate'], 2, '.', '');
					} ?>
				</span>
			</td>
			...
			<?php } ?>
			<td><a href="admin.php?page=adrotate-advertiser&view=message&request=renew&id=<?php echo $ad['id']; ?>&_wpnonce=<?php echo $nonce; ?>"><?php _e('Renew', 'adrotate-pro'); ?></a> - <a href="admin.php?page=adrotate-advertiser&view=message&request=remove&id=<?php echo $ad['id']; ?>&_wpnonce=<?php echo $nonce; ?>"><?php _e('Remove', 'adrotate-pro'); ?></a> - <a href="admin.php?page=adrotate-advertiser&view=message&request=other&id=<?php echo $ad['id']; ?>&_wpnonce=<?php echo $nonce; ?>"><?php _e('Other', 'adrotate-pro'); ?></a></td>
		</tr>
		<?php } ?>
	</tbody>
