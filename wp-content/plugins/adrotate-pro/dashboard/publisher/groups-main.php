...
------------------------------------------------------------------------------------ */
?>
<h3><?php _e('Manage Groups', 'adrotate-pro'); ?></h3>
	...
	<div class="tablenav">
		<div class="alignleft">
			<select name="adrotate_action" id="cat" class="postform">
				...
				<option value="group_delete_banners"><?php _e('Delete Group including ads', 'adrotate-pro'); ?></option>
			</select>
   	...
   	<table class="widefat tablesorter manage-groups-main" style="margin-top: .5em">
		...
		if($groups) {
			...
			foreach($groups as $group) {
		        ...
		        if($group->fallback > 0) $modus[] = __('Fallback', 'adrotate-pro').' (Group '.$group->fallback.')';
			    ...
			    <tr class='<?php echo $class; ?>'>
					...
					<td>
						<strong><a class="row-title" href="<?php echo admin_url('/admin.php?page=adrotate-groups&view=edit&group='.$group->id);?>" title="<?php _e('Edit', 'adrotate-pro'); ?>"><?php echo $group->name;?></a></strong> <?php if($adrotate_config['stats'] == 1) { ?>- <a href="<?php echo admin_url('/admin.php?page=adrotate-groups&view=report&group='.$group->id);?>" title="<?php _e('Stats', 'adrotate-pro'); ?>"><?php _e('Stats', 'adrotate-pro'); ?></a><?php } ?>
		...
		<?php } else { ?>
				<tr>
					...
					<td colspan="<?php echo ($adrotate_config['stats'] == 1) ? '9' : '5'; ?>"><em><?php _e('No groups created!', 'adrotate-pro'); ?></em></td>
				</tr>
		<?php } ?>
			</tbody>
	</table>
