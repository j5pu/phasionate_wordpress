<?php 
	...
	if(!$is_new){
		if(!($list = $this->get($id, NULL, true))){
			wp_die('<h2>'._('This list does not exist or has been deleted!', 'mymail').'</h2>');
		}
...
<table class="form-table">
	...
	<tr>
		...
		<td>
			<div class="detail">
				<ul class="click-to-edit">
					<li><?php echo esc_attr( $list->description ); ?>&nbsp;</li>
					<li><textarea id="description" class="widefat" type="text" name="mymail_data[description]"><?php echo esc_attr( $list->description ); ?></textarea></li>
				</ul>
			</div>
		</td>
	</tr>
