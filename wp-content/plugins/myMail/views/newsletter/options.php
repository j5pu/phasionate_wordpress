...
<?php if($editable) :?>
	...
	?>
	<ul class="colors<?php if(count(array_count_values($original_names)) > 1) echo ' has-labels'; ?>">
	<?php 
...
<?php 
	if($backgroundlinks = $this->templateobj->get_background_links($post->post_content)) :
		?> 	<label><?php _e('Background Links', 'mymail') ?></label><br> <ul class="backgroundlinks"> <?php
		foreach($backgroundlinks as $link){
			?>
			<li><input type="text" class="background-link widefat" value="<?php echo $link?>" data-value="<?php echo $link?>"></li>
			<?php
		}
		?> </ul> <?php
	endif;
