...
//modified for the MyMail Newsletter plugin by revaxarts
...
class mymail_GeoIP {
	...
	function __construct($filename, $flags = '') {
		if(empty($flags)) $flags = GEOIP_STANDARD;
	...
	function geoip_country_id_by_name_v6($name) {
	...
	function geoip_country_code_by_name_v6($name) {
	...
	function geoip_country_name_by_name_v6($name) {
		$country_id = $this->geoip_country_id_by_name_v6($name);
		if ($country_id !== false) {
	...
	function geoip_country_id_by_addr_v6($addr) {
	...
	function geoip_country_code_by_addr_v6($addr) {
	...
	function geoip_country_name_by_addr_v6($addr) {
			$country_id = $this->geoip_country_id_by_addr_v6($addr);
			if ($country_id !== false) {
				return $this->GEOIP_COUNTRY_NAMES[$country_id];
			}
	...
	function _common_get_org($seek_org){
	...
	function geoip_name_by_addr_v6 ($addr) {
		if ($addr == NULL) {
			return 0;
		}
		$ipnum = inet_pton($addr);
