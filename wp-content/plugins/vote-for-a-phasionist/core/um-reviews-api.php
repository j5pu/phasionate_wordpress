...
class UM_Reviews_API_Core {
	...
	***/
	function adjust_rating( $postid, $old_rating, $new_rating ) {
		...
		if ( $reviews_compound > 0 ) {
			$reviews_avg = $reviews_total;
			$reviews_avg = number_format( $reviews_avg, 2 );
			update_user_meta( $user_id, '_reviews_total', $reviews_avg );
		}
	...
	***/
	function publish_review( $postid ) {
		...
		if ( $reviews_compound > 0 ) {
			...
			update_user_meta( $user_id, '_reviews_total', number_format( 0, 2 ) );
		}
	...
	***/
	function rating_header() {
		$result = ( um_is_myprofile() ) ? __('Tus votos','um-reviews') : __('Tus puntos:','um-reviews');
		return $result;
	}
	...
	***/
	function avg_rating( $user_id = null ) {
		...
		$avg_rating = (double) get_user_meta( $user_id, '_reviews_total', true );
		
		if ( $total_ratings == 1 ) {
			$result = sprintf(__('Le ha votado %s phasionista.','um-reviews'), number_format( $total_ratings ) );
		} else {
			$result = sprintf(__('Le han votado %s phasionistas.','um-reviews'), number_format( $total_ratings ) );
		}
		
		if ( $total_ratings == 0 && um_is_myprofile() )
			$result = __('No has conseguido votos. ¡Pide ayuda a un amigo!','um-reviews');
		
		if ( $total_ratings == 0 && !um_is_myprofile() )
			$result = __('Nadie ha votado a este usuario','um-reviews');
	...
	***/
	function get_rating( $user_id = null ) {
		$user_id = ( $user_id ) ? $user_id : um_profile_id();
		$result = (double) get_user_meta( $user_id, '_reviews_total', true );
	...
	***/
	...
	function is_a_player($user_id){
		...
		if ( $role[0] == 'concursante-portada-mayo-2015'){
			return true;
		}else{
			return false;
		}
